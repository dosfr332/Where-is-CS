...
const standardRegEx: RegExp = /^(?<Lat>-?(?:[0-9]|[1-8][0-9]|90)(?:\.\d{1,6})?)[,;]?\s(?<Long>-?(?:[0-9]|[1-9][0-9]|1[0-7][0-9]|180)(?:\.\d{1,6})?)(?<label>\s[a-zs A-Z\s]*)?$/;
...
const dmRegEx: RegExp = /^(?<ddmLat>-?(?:[0-9]|[1-8][0-9]|90))(?:[°d]\s?|\s)(?<ddmLatMin>[0-5]?[0-9](?:\.\d{1,6})?)(?:['"′m]?\s?)(?<ddmLatDir>[NS])?,?\s+(?<ddmLong>-?(?:[0-9]|[1-9][0-9]|1[0-7][0-9]|180))(?:[°d]\s?|\s)(?<ddmLongMin>[0-5]?[0-9](?:\.\d{1,6})?)(?:['"′m]?\s?)(?<ddmLongDir>[EW])?(?<label>\s[a-zs A-Z\s]*)?$/
const directionRegEx: RegExp = /^(?<dirLat>(?:[0-9]|[1-8][0-9]|90)(?:\.\d{1,6})?)['"]?\s*(?<dirLatDir>[NS]),?\s*(?<dirLong>(?:[0-9]|[1-9][0-9]|1[0-7][0-9]|180)(?:\.\d{1,6})?)['"]?\s?(?<dirLongDir>[EW])(?<label>\s[a-zs A-Z\s]*)?$/
